...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;//单位阵,vio的值一直没变
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        //ROS_INFO_STREAM("w_r_vio"<<w_r_vio.matrix());
        //ROS_INFO_STREAM("w_t_vio"<<w_t_vio);
        //record the cost values before and after optimization
        double before_sequential_p = 0;
        double before_sequential_q = 0;
        double after_sequential_p = 0;
        double after_sequential_q = 0;

        double before_loop_p = 0;
        double before_loop_q = 0;
        double after_loop_p = 0;
        double after_loop_q = 0;
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                for (int j = 1; j < 5; j++)
                {
                  if (i - j >= 0 && sequence_array[i] == sequence_array[i-j])
                  {
                    ...
                    //calculate sequntial edge residual manually
                    Vector3d res;
                    Vector3d pi = Vector3d( t_array[i-j][0],  t_array[i-j][1],  t_array[i-j][2]);
                    Vector3d pj = Vector3d( t_array[i][0],  t_array[i][1],  t_array[i][2]);
                    res = relative_t - q_array[i-j].inverse() * (pj-pi);
                    before_sequential_p += 0.5*res.transpose()*res;
                    double q_cost = NormalizeAngle(euler_array[i][0] - euler_array[i-j][0] - relative_yaw);
                    before_sequential_q += 0.5*q_cost*q_cost;
                  }
                }
                ...
                if((*it)->has_loop)
                {
                    ...
                    Vector3d relative_t;
                    relative_t = (*it)->getLoopRelativeT();//loop_info里装的是由pnp求得的回环间的相对位姿
                    ...
                    //calculate sequntial edge residual manually
                    Vector3d res;
                    Vector3d pi = Vector3d( t_array[connected_index][0],  t_array[connected_index][1],  t_array[connected_index][2]);
                    Vector3d pj = Vector3d( t_array[i][0],  t_array[i][1],  t_array[i][2]);
                    res = relative_t - q_array[connected_index].inverse() * (pj-pi);
                    before_loop_p += 0.5*res.transpose()*res;
                    double q_cost = NormalizeAngle(euler_array[i][0] - euler_array[connected_index][0] - relative_yaw) * 0;
                    before_loop_q += 0.5*q_cost*q_cost;
                }
            ...
            */
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                //calculate cost value after optimization
                //sequntial edgs
                Matrix3d j_r;
                Vector3d j_t;
                (*it)->getVioPose(j_t, j_r);
                Vector3d euler_j = Utility::R2ypr(j_r);
                auto jt = it;
                for (int j = 1; j < 5; j++)
                {
                    if (i - j >= 0 && sequence_array[i] == sequence_array[i-j])
                    {   jt--;
                        Matrix3d i_r;
                        Vector3d i_t;
                        (*jt)->getVioPose(i_t, i_r); 
                        Vector3d euler_i = Utility::R2ypr(i_r);
                        Vector3d relative_t = j_t -i_t;
                        //sonyROS_INFO_STREAM("relative_t"<<relative_t);
                        relative_t = i_r.transpose() * relative_t;
                        
                        double relative_yaw = euler_j[0] - euler_i[0];                        
                        //calculate sequntial edge residual manually
                        Vector3d res;
                        Vector3d pi = Vector3d( t_array[i-j][0],  t_array[i-j][1],  t_array[i-j][2]);
                        Vector3d pj = Vector3d( t_array[i][0],  t_array[i][1],  t_array[i][2]);
                        Quaterniond tmp_q;//更新后的i的姿态
                        tmp_q = Utility::ypr2R(Vector3d(euler_array[i-j][0], euler_array[i-j][1], euler_array[i-j][2]));
                        res = relative_t - tmp_q.normalized().conjugate() * (pj-pi);
                        //ROS_INFO_STREAM("res"<<res);
                        after_sequential_p += 0.5*res.transpose()*res;
                        double q_cost = NormalizeAngle(euler_array[i][0] - euler_array[i-j][0] - relative_yaw);
                        after_sequential_q += 0.5*q_cost*q_cost;
                    }
                }
                //loop cost
                if((*it)->has_loop)
                {
                    ...
                    Vector3d pj = Vector3d( t_array[i][0],  t_array[i][1],  t_array[i][2]);
                    Quaterniond tmp_q;//更新后的i的姿态
                    tmp_q = Utility::ypr2R(Vector3d(euler_array[connected_index][0], euler_array[connected_index][1], euler_array[connected_index][2]));
                    res = relative_t - tmp_q.inverse() * (pj-pi);
                    after_loop_p += 0.5*res.transpose()*res;
                    double q_cost = NormalizeAngle(euler_array[i][0] - euler_array[connected_index][0] - relative_yaw) * 0;
                    after_loop_q += 0.5*q_cost*q_cost;
                }
            ...
            cout << setiosflags(ios::left) << setw(10) << " " << resetiosflags(ios::left) // 用完之后清除
                << setiosflags(ios::right)<< setw(15) << "iteration_n" << setw(15) << "seq_p" << setw(15)  << "seq_q"<< setw(15) << "loop_p"<< setw(15) << "loop_q"
                << setw(15) << "sum_seq"<< setw(15) << "sum_loop"<< setw(15) << "sum"
                << resetiosflags(ios::right) << endl;
            cout<<"------------------------------------------------------------------------------------------------------------"<<endl;
            //before optimization
            cout<< setiosflags(ios::left) << setw(10) << "before" << resetiosflags(ios::left) // 用完之后清除
                << setiosflags(ios::right) << setw(15) <<" "<< setw(15) << before_sequential_p << setw(15) <<before_sequential_q
                << setw(15) <<before_loop_p<< setw(15) << before_loop_q
                //sum_seq
                << setw(15) << (before_sequential_p)+(before_sequential_q)
                //sum_loop
                << setw(15) << (before_loop_p)+before_loop_q
                //sum
                << setw(15) << (before_sequential_p)+(before_sequential_q) + (before_loop_p)+before_loop_q
                << resetiosflags(ios::right) << endl;

            //after optimization
            cout<< setiosflags(ios::left) << setw(10) << "after" << resetiosflags(ios::left) // 用完之后清除
                << setiosflags(ios::right) << setw(15) <<" "<< setw(15) << after_sequential_p << setw(15) <<after_sequential_q
                << setw(15) <<after_loop_p<< setw(15) << after_loop_q
                //sum_seq
                << setw(15) << (after_sequential_p)+(after_sequential_q)
                //sum_loop
                << setw(15) << (after_loop_p)+after_loop_q
                //sum
                << setw(15) << (after_sequential_p)+(after_sequential_q) + (after_loop_p)+after_loop_q
                << resetiosflags(ios::right) << endl;
            cout<<"------------------------------------------------------------------------------------------------------------"<<endl;
